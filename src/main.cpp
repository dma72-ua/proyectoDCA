...
static void DrawGoal(const Rectangle& goal) {
    // Palo
    DrawRectangle(goal.x, goal.y - goal.height, 8, goal.height, (Color){180,180,180,255});
    // Bandera
    DrawRectangle(goal.x + 8, goal.y - goal.height + 12, 28, 18, (Color){255, 85, 85, 255});
    DrawTriangle(
        {goal.x + 36, goal.y - goal.height + 21},
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (state == GameState::START) {
            ...
            if (IsKeyPressed(KEY_ENTER)) { resetLevel(); state = GameState::START; }
            if (IsKeyPressed(KEY_R) || IsKeyPressed(KEY_SPACE)) { resetLevel(); state = GameState::PLAYING; }
        ...
        if (state == GameState::PLAYING) {
            ...
            if (!diedThisFrame) {
                if (CheckCollisionRecs(pb, {goal.x - 6, goal.y - goal.height, 40, goal.height})) {
        ...
        // Entidades
        player.Draw();
        for (const auto& en : enemies) en.draw();
        ...
        if (state == GameState::START) {
            ...
            DrawRectangle(0,0, GetScreenWidth(), GetScreenHeight(), (Color){0,255,0,60});
            const char* t1 = "¡VICTORIA!";
            const char* t2 = "ENTER: menu  |  R/ESPACIO: jugar de nuevo";
            int w1 = MeasureText(t1, 48);
            int w2 = MeasureText(t2, 24);
            DrawText(t1, GetScreenWidth()/2 - w1/2 + 2, 150 + 2, 48, BLACK);
            DrawText(t1, GetScreenWidth()/2 - w1/2    , 150    , 48, (Color){255,255,180,255});
            DrawText(t2, GetScreenWidth()/2 - w2/2 + 2, 220 + 2, 24, BLACK);
            DrawText(t2, GetScreenWidth()/2 - w2/2    , 220    , 24, RAYWHITE);
        }
        else if (state == GameState::DEFEAT) {
            DrawRectangle(0,0, GetScreenWidth(), GetScreenHeight(), (Color){255,0,0,60});
            const char* t1 = "DERROTA";
            const char* t2 = "ENTER: menu  |  R/ESPACIO: reintentar";
            int w1 = MeasureText(t1, 48);
            int w2 = MeasureText(t2, 24);
            DrawText(t1, GetScreenWidth()/2 - w1/2 + 2, 150 + 2, 48, BLACK);
            DrawText(t1, GetScreenWidth()/2 - w1/2    , 150    , 48, (Color){255,200,200,255});
            DrawText(t2, GetScreenWidth()/2 - w2/2 + 2, 220 + 2, 24, BLACK);
            DrawText(t2, GetScreenWidth()/2 - w2/2    , 220    , 24, RAYWHITE);
        }
