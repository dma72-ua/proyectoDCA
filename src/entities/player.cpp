...
void Player::updatePlayer(float delta, std::vector<EnvItem> &envItems,
  ...
  // Colisión horizontal
  Rectangle playerRect = {position.x - width * 0.5f, position.y - height, width,
                          height};
  ...
  for (auto &envItem : envItems) {
    if (envItem.blocking && envItem.rect.x <= position.x + width * 0.5f &&
        envItem.rect.x + envItem.rect.width >= position.x - width * 0.5f) {

      // Colisión desde arriba (cayendo)
      if (speed > 0 && position.y <= envItem.rect.y &&
  ...
  if (IsKeyDown(KEY_DOWN) || IsKeyDown(KEY_S)) {
    Rectangle playerRect = {position.x - width * 0.5f, position.y - height,
                            width, height};
...
void Player::draw() {
  ...
  if (texture.id != 0) {
    ...
    Rectangle destRec = {position.x - renderWidth * 0.5f,
